...
function RulesScreen({ onBack }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, backgroundColor: "#0f172a", zIndex: 100, display: "flex", flexDirection: "column" }}>
      <div style={{ backgroundColor: "#0f172a", borderBottom: "1px solid #334155", padding: "calc(env(safe-area-inset-top,44px) + 8px) 16px 12px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={onBack} style={{ background: "none", border: "1px solid #334155", borderRadius: 8, color: "#94a3b8", fontSize: 13, fontWeight: 600, cursor: "pointer", padding: "6px 12px", fontFamily: "inherit" }}>Close</button>
      </div>
...
function HomeScreen({ onCreateRound, onJoinRound, onWatchRound, onAdminLogin, onRejoin, lastRound, savedRounds, onViewHistory, onViewTournaments, onViewRules }) {
        ...
        <div style={{ flex: 1, padding: "0 20px", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr 1fr", gap: 5, marginBottom: 8 }}>
            {[
              ...
              { label: "Rules", icon: "\uD83D\uDCD6", action: onViewRules },
            ].map((btn) => (
...
function SuperAdminScreen({ onLogout, onEnterRound }) {
  ...
  const handleScanScorecard = async (files) => {
    ...
    try {
      ...
      console.error("Scan error:", e);
      setSaScanError("Error: " + (e.message || "Unknown error. Check browser console for details."));
    }
    setSaScanning(false);
  };
  ...
  return (
    <BgScreen bg={BG_OTHER}>
      ...
      <div style={S.content}>
        ...
        {tab === "players" && !loading && (
          <div>
            <div style={{ fontSize: 13, color: "#f8fafc", fontWeight: 600, marginBottom: 16 }}>{allPlayers.length} total players</div>
            {allPlayers.map((p) => {
                ...
                {editingTournamentId === t.id ? (
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: "#94a3b8" }}>{t.rounds?.length || 0} rounds</div>
                    </div>
                    <div style={{ display: "flex", gap: 8 }}>
          ...
          <div>
            ...
            {srScanErr && (
              <div style={{ backgroundColor: "#1c0a0a", border: "1px solid #ef4444", borderRadius: 10, padding: 14, marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 8 }}>Dismiss and try again, or check Netlify logs for more detail.</div>
              </div>
            )}
                ...
                {srResult.lb.map((p, i) => (
                  <div key={p.id} style={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: 10, padding: "12px 14px", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ fontSize: 11, color: "#94a3b8" }}>{p.grossTotal} gross</div>
                    </div>
                  </div>
                ))}
              ...
              <div>
                ...
                {SR_GAME_TYPES.map((gt) => (
                  <button key={gt} onClick={() => setSrGameType(gt)} style={{ width: "100%", textAlign: "left", backgroundColor: srGameType === gt ? "#022c22" : "#1e293b", border: srGameType === gt ? "1px solid #22c55e" : "1px solid #334155", borderRadius: 10, padding: "10px 14px", marginBottom: 8, cursor: "pointer", fontFamily: "inherit" }}>
                    ...
                    <div style={{ fontSize: 11, color: "#94a3b8" }}>{GAME_TYPES[gt]?.description}</div>
                  </button>
                ))}
                      ...
                      {nine.map((hs, idx) => {
                        ...
                        return (
                          <div key={hs.hole_number} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                            <div style={{ fontSize: 11, color: "#94a3b8" }}>H{hs.hole_number} <span style={{ color: "#475569" }}>p{courseHole?.par || "-"}</span></div>
                            <input
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.85)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, maxWidth: 480, margin: "0 auto" }}>
      <div style={{ backgroundColor: "#1e293b", borderRadius: 20, padding: 28, width: "100%", border: "1px solid #334155", textAlign: "center" }}>
        ...
        <p style={{ fontSize: 14, color: "#94a3b8", margin: "0 0 20px" }}>{round.course_name} · {GAME_TYPES[round.game_type]?.label}</p>
        ...
        <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.85)", zIndex: 100, display: "flex", flexDirection: "column" }}>
          ...
          {isCreator ? (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
              {/* Same tabs as in-game admin */}
              <div style={{ display: "flex", gap: 6, padding: "10px 16px", backgroundColor: "#0f172a" }}>
                {[["scores","Scores"],["players","Players"],["pars","Pars"],["indexes","Indexes"]].map(([key,label]) => (
                  <button key={key} onClick={() => setGameAdminTab(key)} style={{ flex: 1, padding: "8px 4px", borderRadius: 8, border: "none", backgroundColor: gameAdminTab === key ? "#22c55e" : "#1e293b", color: gameAdminTab === key ? "#0f172a" : "#94a3b8", fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>{label}</button>
                ))}
              </div>
              <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
                ...
                {/* PLAYERS TAB */}
                {gameAdminTab === "players" && (
                  <div>
                    <div style={{ fontSize: 13, color: "#94a3b8", marginBottom: 12 }}>Edit name, handicap, or remove. This round only.</div>
                    {round.game_type !== "banker" ? (
                      ...
                      <div style={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: 12, padding: 14, marginBottom: 12 }}>
                        <div style={{ fontSize: 12, color: "#f59e0b" }}>Players cannot be added to a Banker round once started.</div>
                      </div>
                    )}
                    {players.map(p => (
                ...
                {gameAdminTab === "pars" && (
                  <div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
                      {[holes.slice(0,9), holes.slice(9,18)].map((nine, ni) => (
                        <div key={ni} style={{ backgroundColor: "#0f172a", borderRadius: 8, padding: 8 }}>
                          <div style={{ fontSize: 9, fontWeight: 800, color: ni === 0 ? "#22c55e" : "#3b82f6", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8, textAlign: "center" }}>{ni === 0 ? "Front 9" : "Back 9"}</div>
                          {nine.map((h) => {
                            const idx = holes.findIndex(x => x.hole_number === h.hole_number);
                            return (
                              <div key={h.hole_number} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                                <div style={{ fontSize: 11, color: "#f8fafc", fontWeight: 600 }}>H{h.hole_number}</div>
                                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                  <button onClick={() => { const u=[...adminEditHoles||holes]; if(u[idx]) { u[idx]={...u[idx],par:Math.max(3,u[idx].par-1)}; setAdminEditHoles(u); }}} style={{ width:26,height:26,borderRadius:6,border:"1px solid #334155",backgroundColor:"#1e293b",color:"#f8fafc",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center" }}>-</button>
                                  <div style={{ fontSize:15,fontWeight:800,color:"#22c55e",width:18,textAlign:"center" }}>{(adminEditHoles||holes)[idx]?.par}</div>
                                  <button onClick={() => { const u=[...adminEditHoles||holes]; if(u[idx]) { u[idx]={...u[idx],par:Math.min(6,u[idx].par+1)}; setAdminEditHoles(u); }}} style={{ width:26,height:26,borderRadius:6,border:"1px solid #334155",backgroundColor:"#1e293b",color:"#f8fafc",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center" }}>+</button>
                                </div>
                              </div>
                            );
                          })}
                          ...
                          {nine.map((h) => {
                            ...
                            return (
                              <div key={h.hole_number} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                                <div style={{ fontSize: 11, color: "#f8fafc", fontWeight: 600 }}>H{h.hole_number}</div>
                                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                  <button onClick={() => { const u=[...adminEditHoles||holes]; if(u[idx]) { u[idx]={...u[idx],stroke_index:Math.max(1,u[idx].stroke_index-1)}; setAdminEditHoles(u); }}} style={{ width:26,height:26,borderRadius:6,border:"1px solid #334155",backgroundColor:"#1e293b",color:"#f8fafc",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center" }}>-</button>
                                  <div style={{ fontSize:15,fontWeight:800,color:"#f59e0b",width:22,textAlign:"center" }}>{(adminEditHoles||holes)[idx]?.stroke_index}</div>
                                  <button onClick={() => { const u=[...adminEditHoles||holes]; if(u[idx]) { u[idx]={...u[idx],stroke_index:Math.min(18,u[idx].stroke_index+1)}; setAdminEditHoles(u); }}} style={{ width:26,height:26,borderRadius:6,border:"1px solid #334155",backgroundColor:"#1e293b",color:"#f8fafc",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center" }}>+</button>
                                </div>
                              </div>
                            );
                          })}
...
function ScorecardScreen({ round, me, onViewDashboard, isSpectator }) {
  ...
  const [navPadding, setNavPadding] = useState(150);
  useEffect(() => {
    const measure = () => {
      const master = document.getElementById("ff-master-scroll");
      if (master && master.clientWidth > 0) {
        setNavPadding(Math.max(0, (master.clientWidth / 2) - 20));
      }
    };
    measure();
  ...
  const [adminSaving, setAdminSaving] = useState(false);
  const [adminMsg, setAdminMsg] = useState("");
  const [adminEditingPlayer, setAdminEditingPlayer] = useState(null);
  const [adminEditPlayerName, setAdminEditPlayerName] = useState("");
    ...
    <BgScreen bg={BG_OTHER}>
        ...
        <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.85)", zIndex: 100, display: "flex", flexDirection: "column" }}>
          ...
          {!gameAdminAuthed ? (
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", gap: 6, padding: "10px 16px", backgroundColor: "#0f172a" }}>
                {[["scores","Scores"],["players","Players"],["pars","Pars"],["indexes","Indexes"]].map(([key,label]) => (
                  <button key={key} onClick={() => { setGameAdminTab(key); setAdminEditingPlayer(null); setAdminConfirmRemove(null); }} style={{ flex: 1, padding: "8px 4px", borderRadius: 8, border: "none", backgroundColor: gameAdminTab === key ? "#22c55e" : "#1e293b", color: gameAdminTab === key ? "#0f172a" : "#94a3b8", fontWeight: 700, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>{label}</button>
                ))}
              </div>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
                ...
                {gameAdminTab === "players" && (
                  <div>
                    <div style={{ fontSize: 13, color: "#94a3b8", marginBottom: 12 }}>Edit name, handicap, or remove a player. This round only.</div>

                    {adminConfirmRemove && (
                      <div style={{ backgroundColor: "#1c0a0a", border: "1px solid #ef4444", borderRadius: 10, padding: 14, marginBottom: 16 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: "#ef4444", marginBottom: 8 }}>Remove {adminConfirmRemove.name} from this round?</div>
                        <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 12 }}>This will delete the player and all their scores from this round.</div>
                        <div style={{ display: "flex", gap: 8 }}>
                          <button onClick={async () => {
                            setAdminSaving(true);
                            try {
                              await supabase.from("scores").delete().eq("player_id", adminConfirmRemove.id).eq("round_id", round.id);
                              await supabase.from("players").delete().eq("id", adminConfirmRemove.id);
                              setOthers(prev => prev.filter(p => p.id !== adminConfirmRemove.id));
                              const fresh = await dbGetScores(round.id);
                              setAllScores(fresh);
                              setAdminConfirmRemove(null);
                              setAdminMsg("Player removed.");
                              setTimeout(() => setAdminMsg(""), 2000);
                            } catch(e) { setAdminMsg("Error: " + e.message); }
                            setAdminSaving(false);
                          }} style={{ flex: 1, backgroundColor: "#ef4444", color: "#fff", border: "none", borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                            Yes, Remove
                          </button>
                          <button onClick={() => setAdminConfirmRemove(null)} style={{ flex: 1, backgroundColor: "transparent", color: "#94a3b8", border: "1px solid #334155", borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
                        </div>
                      </div>
                    )}
                    ...
                    {adminEditingPlayer ? (
                      <div style={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: 12, padding: 14, marginBottom: 12 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: "#f8fafc", marginBottom: 10 }}>Edit {adminEditingPlayer.name}</div>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          <button onClick={async () => {
                            ...
                            setAdminSaving(false);
                          }} style={{ flex: 1, backgroundColor: "#22c55e", color: "#0f172a", border: "none", borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Save</button>
                          <button onClick={() => setAdminEditingPlayer(null)} style={{ flex: 1, backgroundColor: "transparent", color: "#94a3b8", border: "1px solid #334155", borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
                        </div>
                      </div>
                    ) : (
                      [...others, me].map((player) => (
                        <div key={player.id} style={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: 12, padding: "12px 14px", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          ...
                          <div style={{ display: "flex", gap: 6 }}>
                            <button onClick={() => { setAdminEditingPlayer(player); setAdminEditPlayerName(player.name?.replace(" (Guest)", "") || player.name); setAdminEditPlayerHcp(String(player.handicap)); }}
                              style={{ backgroundColor: "#0f172a", border: "1px solid #334155", borderRadius: 8, color: "#f8fafc", fontSize: 12, fontWeight: 600, cursor: "pointer", padding: "6px 10px", fontFamily: "inherit" }}>✏️ Edit</button>
                            {player.id !== me.id && (
                              <button onClick={() => setAdminConfirmRemove(player)}
                                style={{ backgroundColor: "#1c0a0a", border: "1px solid #ef4444", borderRadius: 8, color: "#ef4444", fontSize: 12, fontWeight: 700, cursor: "pointer", padding: "6px 10px", fontFamily: "inherit" }}>✕ Remove</button>
                            )}
                          </div>
                        </div>
                      ))
                ...
                {gameAdminTab === "pars" && (
                  <div>
                    <div style={{ fontSize: 13, color: "#94a3b8", marginBottom: 12 }}>Change par for any hole. This round only.</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
                      {[adminEditHoles.slice(0,9), adminEditHoles.slice(9,18)].map((nine, ni) => (
                        <div key={ni} style={{ backgroundColor: "#0f172a", borderRadius: 8, padding: 8 }}>
                          <div style={{ fontSize: 9, fontWeight: 800, color: ni === 0 ? "#22c55e" : "#3b82f6", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8, textAlign: "center" }}>{ni === 0 ? "Front 9" : "Back 9"}</div>
                          {nine.map((h, localI) => { const i = ni * 9 + localI; return (
                            <div key={h.hole_number} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                              <div style={{ fontSize: 11, color: "#f8fafc", fontWeight: 600 }}>H{h.hole_number}</div>
                              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                                <button onClick={() => { const u=[...adminEditHoles]; u[i]={...u[i],par:Math.max(3,u[i].par-1)}; setAdminEditHoles(u); }} style={{ width:26,height:26,borderRadius:6,border:"1px solid #334155",backgroundColor:"#1e293b",color:"#f8fafc",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center" }}>-</button>
                                <div style={{ fontSize:15,fontWeight:800,color:"#22c55e",width:18,textAlign:"center" }}>{h.par}</div>
                                <button onClick={() => { const u=[...adminEditHoles]; u[i]={...u[i],par:Math.min(6,u[i].par+1)}; setAdminEditHoles(u); }} style={{ width:26,height:26,borderRadius:6,border:"1px solid #334155",backgroundColor:"#1e293b",color:"#f8fafc",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center" }}>+</button>
                              </div>
                            </div>
                          ); })}
                        </div>
                        ...
                        <div key={ni} style={{ backgroundColor: "#0f172a", borderRadius: 8, padding: 8 }}>
                          ...
                          {nine.map((h, localI) => { const i = ni * 9 + localI; return (
                            <div key={h.hole_number} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                              <div style={{ fontSize: 11, color: "#f8fafc", fontWeight: 600 }}>H{h.hole_number}</div>
                              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                                <button onClick={() => { const u=[...adminEditHoles]; u[i]={...u[i],stroke_index:Math.max(1,u[i].stroke_index-1)}; setAdminEditHoles(u); }} style={{ width:26,height:26,borderRadius:6,border:"1px solid #334155",backgroundColor:"#1e293b",color:"#f8fafc",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center" }}>-</button>
                                <div style={{ fontSize:15,fontWeight:800,color:"#f59e0b",width:22,textAlign:"center" }}>{h.stroke_index}</div>
                                <button onClick={() => { const u=[...adminEditHoles]; u[i]={...u[i],stroke_index:Math.min(18,u[i].stroke_index+1)}; setAdminEditHoles(u); }} style={{ width:26,height:26,borderRadius:6,border:"1px solid #334155",backgroundColor:"#1e293b",color:"#f8fafc",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center" }}>+</button>
                              </div>
                            </div>
                          ); })}
                        </div>
      ...
      <div style={{ backgroundColor: "#1e293b", borderBottom: "1px solid #334155", padding: "10px 16px", position: "sticky", top: 88, zIndex: 20 }}>
        <div id="ff-master-scroll" style={S.holeNav} onScroll={(e) => {
          ...
          <div style={{ flexShrink: 0, width: navPadding }} />
          {holes.map((h) => {
        ...
        {curHole && (
          <div style={{ textAlign: "center", paddingTop: 4 }}>
            <div style={{ fontSize: 20, fontWeight: 900, color: "#f8fafc", lineHeight: 1.1 }}>Par {curHole.par}</div>
            <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600, marginTop: 2 }}>Stroke Index {curHole.stroke_index}</div>
          </div>
        )}
      </div>
      ...
      {curHole && (
        <div style={S.holeCard}>
          ...
          {!isSpectator && curHole && round.game_type !== "banker" && (() => {
            ...
            const shortName = me.name.split(" ")[0];
            let totalLabel, totalVal, totalColor;
            if (round.game_type === "stableford") {
              totalLabel = "Points"; totalVal = myStablefordTotal + " pts"; totalColor = "#22c55e";
            } else if (round.game_type === "matchplay" || round.game_type === "matchplay_teams") {
              const lead = Math.floor(myMatchTotal);
              totalLabel = "Match"; totalVal = lead === 0 ? "0 pts" : lead + " pts"; totalColor = lead > 0 ? "#22c55e" : "#94a3b8";
            } else if (round.game_type === "banker") {
              totalLabel = "Balance"; totalVal = (myBankerTotal >= 0 ? "+$" : "-$") + Math.abs(myBankerTotal); totalColor = myBankerTotal > 0 ? "#22c55e" : myBankerTotal < 0 ? "#ef4444" : "#94a3b8";
            } else {
              totalLabel = "Gross"; totalVal = myToPar === 0 ? "E" : myToPar > 0 ? "+" + myToPar : "" + myToPar; totalColor = myToPar < 0 ? "#22c55e" : myToPar > 0 ? "#ef4444" : "#94a3b8";
            }
            return (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 12, borderTop: "1px solid #334155", marginTop: 4, gap: 8 }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: "#22c55e", textTransform: "uppercase", flexShrink: 0 }}>{shortName}</div>
                <div style={{ display: "flex", gap: 14, alignItems: "center", flex: 1, justifyContent: "center" }}>
                  {round.game_type !== "stroke" && (
                    <>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ fontSize: 20, fontWeight: 900, color: myToPar < 0 ? "#22c55e" : myToPar > 0 ? "#ef4444" : "#94a3b8", lineHeight: 1 }}>{myToPar === 0 ? "E" : myToPar > 0 ? "+" + myToPar : myToPar}</div>
                        <div style={{ fontSize: 9, color: "#64748b", textTransform: "uppercase", letterSpacing: 0.5, marginTop: 2 }}>To Par</div>
                      </div>
                      <div style={{ width: 1, height: 32, background: "#334155" }} />
                    </>
                  )}
                  <div style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 20, fontWeight: 900, color: "#f8fafc", lineHeight: 1 }}>{myGross || "—"}</div>
                    <div style={{ fontSize: 9, color: "#64748b", textTransform: "uppercase", letterSpacing: 0.5, marginTop: 2 }}>Strokes</div>
                  </div>
                </div>
                <div style={{ backgroundColor: "#0f172a", border: "1px solid #334155", borderRadius: 10, padding: "6px 12px", textAlign: "center", flexShrink: 0 }}>
                  <div style={{ fontSize: 9, color: "#64748b", fontWeight: 700, textTransform: "uppercase" }}>{totalLabel}</div>
                  <div style={{ fontSize: 18, fontWeight: 800, color: totalColor }}>{totalVal}</div>
                </div>
              </div>
            );
          })()}
          ...
          <div
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 4, overflow: "hidden" }}>
              {holes.map((h) => (
                <div key={"dot"+h.hole_number} onClick={() => { setActiveHole(h.hole_number); requestAnimationFrame(() => centreHoleNav(h.hole_number)); }}
                  style={{ width: h.hole_number === activeHole ? 16 : 5, height: 5, borderRadius: 3, backgroundColor: h.hole_number === activeHole ? "#22c55e" : "#334155", cursor: "pointer", flexShrink: 0, transition: "width 0.2s" }} />
              ))}
            </div>
            <button onClick={() => { if (activeHole < holes.length) { const next = activeHole + 1; setActiveHole(next); requestAnimationFrame(() => centreHoleNav(next)); } }}
              style={{ background: "none", border: "none", color: activeHole < holes.length ? "#94a3b8" : "#334155", fontSize: 20, fontWeight: 900, cursor: activeHole < holes.length ? "pointer" : "default", padding: "4px 8px", fontFamily: "inherit" }}>›</button>
          </div>
          <div style={{ textAlign: "center", fontSize: 9, color: "#475569", fontWeight: 600, letterSpacing: 0.5, marginBottom: 8 }}>‹ SWIPE TO CHANGE HOLE ›</div>
        </div>
      )}
...
export default function GolfApp() {
  ...
  return (
    <div style={S.app}>
      {screen === "rules_standalone" && <RulesScreen onBack={() => setScreen("home")} />}
      {screen === "home" && <HomeScreen onCreateRound={() => setScreen("create")} onJoinRound={() => setScreen("join")} onWatchRound={() => setScreen("watch")} onAdminLogin={() => setScreen("admin_login")} onRejoin={handleRejoin} lastRound={lastRound} savedRounds={savedRounds} onViewHistory={() => setScreen("history")} onViewTournaments={() => setScreen("tournaments")} onViewRules={() => setScreen("rules_standalone")} />}
      {screen === "history" && <PastRoundsScreen onBack={() => setScreen("home")} onViewRound={(r) => {
        if (r._rejoinPlayer && r._rejoinRound) {
          setRound(r._rejoinRound);
          setMe(r._rejoinPlayer);
          setScreen("dashboard");
        } else {
          setViewingRound(r);
          setScreen("view_round");
        }
      }} />}
